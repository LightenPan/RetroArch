...
static bool playlist_core_path_equal(const char *real_core_path, const char *entry_core_path, const playlist_config_t *config)
{
   ...
   if (!string_is_equal(entry_real_core_path, file_path_str(FILE_PATH_DETECT)) &&
       !string_is_equal(entry_real_core_path, file_path_str(FILE_PATH_BUILTIN)))
...
// MG 获取rom的实际路径，如果下载目录有，则优先用下载目录
...
void playlist_set_last_select_ptr(playlist_t *playlist, size_t select_ptr)
{
   ...
   if (playlist_get_size(playlist) == 0)
      return ;

   if (select_ptr > playlist_get_size(playlist))
   {
      playlist->last_select_ptr = playlist_get_size(playlist) - 1;
      return ;
   }
